...
function updateDepth(book, newPage) {
	...
	if (newPage>3)
		...
		$('.agenda .p2 .depth').css({width: 0});

		depthWidth = 16*Math.min(1, (pages-page)*2/pages);

	if (newPage<pages-3)
		$('.agenda .p11 .depth').css({
			width: depthWidth,
			right: 20 - depthWidth
		});
	else
		$('.agenda .p11 .depth').css({width: 0});
...
function loadPage(page) {
	...
	$.ajax({url: 'pages/page' + page + '.html'}).
			...
			$('.agenda .p' + page).html(pageHtml.replace('../agenda/', ''));
		});
...
function updateTabs() {
	...
	var tabs = {2: 'Hoteles', 4:'Tours', 6:'Transporte', 8:'Serv Turisticos', 10:'Checkout'},
			left = [],
			right = [],
			book = $('.agenda'),
	...
	$('.agenda .tabs .right').html(right.join(''));
	$('.agenda .tabs .right').html(right.join(''));
...
function zoomThis(pic) {
	...
	var	position, translate,
		...
		$('.agenda').data().zoomIn = true;

		$('.agenda').turn('disable', true);
...
function zoomOut() {
	...
	var transitionEnd = $.cssTransitionEnd(),
		completeTransition = function(e) {
			$('#book-zoom').unbind(transitionEnd);
			$('.agenda').turn('disable', false);
			$('body').css({'overflow': 'auto'});
			moveBar(false);
		};

	$('.agenda').data().zoomIn = false;
